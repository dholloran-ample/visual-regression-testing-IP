...
export class GlobalNav {
  ...
  containerClass(containerName, navName) {
    let classes = [containerName];
  ...
  //TODO can this be included in toggleMenu?
  ...
  //TODO maybe use this to handle profile click too?
  private toggleMenuAuthCheck(navName, navRequiresAuth) {
    if (this.openNavName === navName) {
      this.openNavName = ''; //Menu can always close
    } else if (navRequiresAuth) {
      this.openNavName = this.auth.authenticated ? navName : ''; //Only open menu if authed
    } else {
  ...
  getDataLabel(container){
    if (container['nav-name'] === 'profile-nav') {
      if (this.isAuthenticated) {
        return (container.label || {})['authenticated'];
      } else {
        return (container.label || {})['not-authenticated'];
      }
    } else {
  ...
  getContainerHandleClick(event, container){
    if (container['nav-name'] === 'profile-nav') {
      return this.handleProfileClick(event);
    }
    else if (container['nav-name']) {
      return this.toggleMenu(event, container['nav-name']);
    }
    else {
      return;
    }
  ...
  getContainerClassName(container){
    if (container['class-name'] && container['nav-name']) {
      return this.containerClass(container['class-name'], container['nav-name']);
    }
    else if (container['class-name']){
  ...
  renderNavLink(container) {
          //Below working and original
    return (
      <a
        href={container.href}
        data-automation-id={container['automation-id']}
        onClick={event => this.getContainerHandleClick(event, container)}

        data-label={this.getDataLabel(container)}
        class={this.getContainerClassName(container)}
      >
        {this.maybeRenderIcons(container.icons)}
      </a>)
  }
  ...
  //   <nav-link class='menu-container' href='' automationId="sh-menu" data-label='menu' handleOnClick={(event) => {this.toggleMenu(event, 'main-nav')}}>
  //                 {this.maybeRenderIcons(mainMenuIcons)}
  //               </nav-link>
  ...
  giveMenu() {
    ...
    return {
      'class-name': 'give-container',
      'nav-name': 'give-nav',
      'automation-id': 'sh-give',
      'label': 'give',
      'icons': [
        {
          'class': 'donate',
  ...
  //TODO fix issue here - when signed out, getting console error Cannot read property 'avatarUrl' of null
  profileMenu() {
    const avatarUrl = ((this.auth && this.auth.currentUser as any) || {}).avatarUrl;
    const profHTML = `<div class="account-authenticated" style="background-image: url('${avatarUrl}');"/>`;
    const accountIcon =
    ...
    return {
      'class-name': 'profile-container',
      'nav-name': 'profile-nav',
      'href': `${this.rootURL()}/signin`,
      'automation-id': 'sh-profile',
      'label': {
      ...
      'icons': [
        {
          'class': 'account',
          'innerHTML': {
            'authenticated': profHTML,
            'not-authenticated': accountIcon
          }
        },
        {
          'class': 'close',
  ...
  //TODO a is links - can use nav-links?
  ...
  render() {
    ...
    let close =
      ...
      // {this.renderNavLink(this.searchLink())} //was
      ...
      //{this.renderNavLink(this.mainMenu())} //was


    const navIsShowing = this.navNames.includes(this.openNavName);
    //TODO renderNavLink the logo link
    return (
      <Fragment>
        <header
          ...
          <div>
            <div class="global-nav-items">
              ...
              <nav-link class='menu-container' href='' automationId="sh-menu" data-label='menu' handleOnClick={(event) => {this.toggleMenu(event, 'main-nav')}}>
                ...
                <nav-link class='search-container' href='/search' automationId="sh-search" data-label='search'>
                  {this.maybeRenderIcons(searchIcon)}
                </nav-link>
              </div>

              <nav-link class="logo" href={this.rootURL()} automationId="sh-logo" innerHTML={crdsLogo}/>

              <div class="user-actions">
                {this.renderNavLink(this.giveMenu())}
                {this.renderNavLink(this.profileMenu())}
