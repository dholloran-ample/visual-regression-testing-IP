...
//TODO need to fix icon spacing and make whole nav-link clickable, not just icon - probs can be fixed with css
@Component({
  tag: 'global-nav',
...
export class GlobalNav {
  ...
  //TODO not liking this - not usable when closing outside button click
  private setOpenNavName(event, newName) {
    event.preventDefault();
    this.openNavName = newName;
  }

  private toggleMenu(event, navName, navRequiresAuth: boolean = false) {
    event.stopPropagation();

    if (this.openNavName === navName) {
      this.setOpenNavName(event, ''); //Nav can always close
    } else if (navRequiresAuth) {
      if (this.isAuthenticated) {
        this.setOpenNavName(event, navName);
      }
    } else {
      this.setOpenNavName(event, navName);
    }
  ...
  closeNav(event) {
    //Don't prevent other links from working
    if(this.navNames.includes(this.openNavName)){
      event.preventDefault();
    }
  ...
  navCloseClasses() {
    const state = this.navNames.includes(this.openNavName) ? ' is-showing' : '';
    return `close-nav${state}`;
  }

  containerClass(className, navName) {
    const state = this.openNavName === navName ? ' nav-is-showing' : '';
    return `${className}${state}`;
  }

  maybeRenderIcons(iconData) {
    if (!Array.isArray(iconData)) return;

    return iconData.map(icon => {
      const innerHTML = () => {
        if (icon.innerHTML && typeof icon.innerHTML === 'object') { //TODO not liking this logic
          return this.isAuthenticated ? icon.innerHTML.authenticated : icon.innerHTML['not-authenticated'];
        }
        return icon.innerHTML;
      }

      return (
        <div class={icon.class} innerHTML={innerHTML()} />
      );
    });
  }
  ...
  render() {
    ...
    const navIsShowing = this.navNames.includes(this.openNavName);
    ...
    return (
      <Fragment>
          ...
          <div>
            ...
            <div class="global-actions">
                <a
                  ...
                  data-label="menu"
                  class={this.containerClass('menu-container', 'main-nav')}
                  onClick={event => this.toggleMenu(event, 'main-nav')}
                >
                  {this.maybeRenderIcons(iconData.main)}
                </a>

                <a href="/search" class="search-container" data-automation-id="sh-search" data-label="search">
                  {this.maybeRenderIcons(iconData.search)}
                </a>
              </div>

              <a href={this.rootURL()} data-automation-id="sh-logo" class="logo" innerHTML={crdsLogo} />
              ...
              <div class="user-actions">
                <a
                  ...
                  data-label="give"
                  class={this.containerClass('give-container', 'give-nav')}
                  onClick={event => this.toggleMenu(event, 'give-nav')}
                >
                  {this.maybeRenderIcons(iconData.give)}
                </a>

                <a
                  href={`${this.rootURL()}/signin`}
                  class={this.containerClass('profile-container', 'profile-nav')}
                  onClick={event => this.toggleMenu(event, 'profile-nav', true)}
                  data-automation-id="sh-profile"
                  data-label={this.isAuthenticated ? 'my account' : 'sign in'}
                >
                  {this.maybeRenderIcons(iconData.profile)}
                </a>
        ...
        <div class={this.navCloseClasses()}>
          <div class="close-nav-icon" innerHTML={this.closeIcon} onClick={this.closeNav.bind(this)} />
        </div>
      </Fragment>
    );
  }
